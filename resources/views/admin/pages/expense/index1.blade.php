...
@section('admin_content')
                    ...
                    <tbody>
                        @foreach ($expenses as $key => $expense)
                            <tr>
                                ...
                                <td>{{ $expense->type }}</td>
                                <td>{{ $expense->employee->name ?? ($expense->counter->name ?? ($expense->vehicle->name ?? ($expense->route->from_location->name .to. $expense->route->toLocation->name ?? 'N/A'))) }}
                                </td>
                                {{-- <td>{{ $expense->employee_name ?? 'N/A' }}</td>
                                <td>{{ $expense->counter ?? 'N/A' }}</td>
                                <td>{{ $expense->vehicle ?? 'N/A' }}</td>
                ...
                <div class="modal-body">
                    <form method="post" action="{{ route('expense.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-12 mb-3">
                                <label for="department" class="form-label">Department</label>
                                <select name="department" id="department" class="form-select" onchange="fetchTypeAndEmployee()" required>
                                    ...
                                    <option value="Helper" {{ old('department') == 'Helper' ? 'selected' : '' }}>Helper</option>
                                    <option value="Supervisor" {{ old('department') == 'Supervisor' ? 'selected' : '' }}>Supervisor</option>
                                    <option value="Route Manager" {{ old('department') == 'Route Manager' ? 'selected' : '' }}>Route Manager</option>
                                    <option value="Owner" {{ old('department') == 'Owner' ? 'selected' : '' }}>Owner</option>
                                    <option value="Counter" {{ old('department') == 'Counter' ? 'selected' : '' }}>Counter</option>
                                    <option value="Vehicle" {{ old('department') == 'Vehicle' ? 'selected' : '' }}>Vehicle</option>
                                    <option value="Route" {{ old('department') == 'Route' ? 'selected' : '' }}>Route</option>
                                </select>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (department) {
                fetchTypeAndEmployee(); // Call function if a department was selected
            }
        });
    </script>
    <script>
        function fetchTypeAndEmployee() {
            ...
            typeSelect.innerHTML = `<option value="" disabled>Select Expense Type</option>`;
            employeeSelect.innerHTML = `<option value="" disabled>Select Employee</option>`;
            employeeField.style.display = 'none';
    
            if (department) {
                fetch(`/get-type-employee?department=${department}`)
                    ...
                    .then(data => {
                        ...
                        // Populate employees if available
                        ...
                        // Show specific fields based on department
                        resetFields();
                        if (["Counter", "Vehicle", "Route"].includes(department)) {
                            document.getElementById(`${department.toLowerCase()}_field`).style.display = 'block';
            ...
            function resetFields() {
                document.getElementById('counter_field').style.display = 'none';
                document.getElementById('vehicle_field').style.display = 'none';
                document.getElementById('route_field').style.display = 'none';
                employeeField.style.display = 'none';
            }
        }
    </script>
